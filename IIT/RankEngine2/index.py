    ...
    def compute_doc_norms(self, index, n_docs, doc_freqs):
        ...
        for term in index.keys():
            for tup in index[term]:
                tfidf_t = ((1 + math.log(tup[1],10)) * math.log(n_docs*1./doc_freqs[term],10)) **2
                if tup[0] in tfidf_ts.keys():
                    tfidf_ts[tup[0]] += tfidf_t
                else:
                    tfidf_ts[tup[0]] = tfidf_t
        for doc_id in tfidf_ts.keys():
            tfidf_ts[doc_id] = math.sqrt(tfidf_ts[doc_id])
        return tfidf_ts
        ...
        for key in index.keys():
            ...
            for d in doc:
                if d[0] in length_dict:
                    length_dict[d[0]] += d[1] * 1.
                else:
                    length_dict[d[0]] = d[1] * 1.
        sum_t = 0
        for key in length_dict.keys():
            sum_t += length_dict[key]
        return  length_dict,sum_t/len(length_dict) * 1.
    ...
    def create_tf_index(self, docs, doc_freqs):
        ...
        for doc in docs:
            for term in list(set(doc)):
                frequency = doc.count(term)*1.
                if term in index:
                    index[term].append([docs.index(doc)+1, frequency])
                else:
                    index[term] = [[docs.index(doc)+1, frequency]]
        return index
